...
/* modified 95/08/09, 2001/03/20 for use in GrADS   -hoop */
        path = getenv("GADDIR") ;
        if (path == NULL || path[0] == 0) {
            path = strcpy(pathbuf, "/usr/local/lib/grads/udunits.dat") ;
        } else {
            strcpy(pathbuf, path) ;
            path = strcat(pathbuf, "/udunits.dat") ;
        }
/* end of modified section for GrADS */
    }
 ...
 * FORTRAN interface to the above functionality.
 */
/* FCALLSCFUN4(INT,utConvert,UTCVT,utcvt, */
 ...
 * FORTRAN interface to the above function.
 */
/* FCALLSCFUN8(INT,utInvCalendar,UTICALTIME,uticaltime, */
/*     INT,INT,INT,INT, */
